...
/**
 * Interface for the IAU1980 nutation parameters.
 */
export interface NutationParams {
    eps : number;
    deps : number;
    dpsi : number;
};
...
export class Nutation {
     ...
     */
    static nutationTerms(T : number) : NutationParams
    {
        const T2 = T * T;
        const T3 = T2 * T;
        ...
        const N = nutationData.length;
        let dpsi = 0.0;
        let deps = 0.0;
        // Note j in (A.25) runs from 1 to N but 0 to N-1 below:
        for (let j = 0; j < N; j++)
        {
            const data = nutationData[j];
